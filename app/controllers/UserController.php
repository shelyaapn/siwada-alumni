...
class UserController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->user_tahun_lulus)){
			$vals = explode("-", str_replace(" ", "", $request->user_tahun_lulus));
			$from = $vals[0];
			$to = $vals[1];
			$db->where("user.tahun_lulus BETWEEN $from AND $to");
		}
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("nama","jenis_kelamin","tahun_lulus","jurusan","email","password","no_telepon","alamat","pekerjaan_saat_ini","photo","account_status","user_role_id");
			...
			//Check if Duplicate Record Already Exit In The Database
			$db->where("no_telepon", $modeldata['no_telepon']);
			if($db->has($tablename)){
     ...
     * @return BaseView
     */
	function alumni($fieldname = null , $fieldvalue = null){
		$request = $this->request;
		$db = $this->GetModel();
		$tablename = $this->tablename;
		$fields = array("user.id_user", 
		...
		$this->view->report_paper_size = "A4";
		$this->view->report_orientation = "portrait";
		$this->render_view("user/alumni.php", $data); //render the full page
	}
	/**
     * List page records
     * @param $fieldname (filter record by a field) 
	...
	function admin($fieldname = null , $fieldvalue = null){
		...
		$this->view->report_paper_size = "A4";
		$this->view->report_orientation = "portrait";
		$this->render_view("user/admin.php", $data); //render the full page
	}
}
